...
const taskLabels: Record<string, { text: string; className: string }> = {
  active: { text: "Completed", className: "bg-success/10 text-success" },
  inactive: { text: "In Progress", className: "bg-warning/10 text-warning" },
};
    ...
    <div className="bg-card border border-border rounded-2xl p-6  animate-fade-in" style={{ animationDelay: "400ms" }}>
      ...
      <div className="space-y-3">
        {users.slice(0, 4).map((user, idx) => {
          const label = taskLabels[user.status] || taskLabels.inactive;
          return (
            <div key={user.id} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground truncate">Working on: {workDescriptions[idx]}</p>
              </div>
